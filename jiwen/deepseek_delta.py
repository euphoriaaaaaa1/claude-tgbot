...
{{
  "connection": <连接需求变化。说完想说的/被热络接住→负；被冷落/敷衍式简短回应→正（亲密进行中回得短≠冷落，见准则）>,
  ...
  "desire":     <**nsfw 张力**变化(只指情色/被撩起，不是普通兴奋)。被撩/调情/亲密升温→正；无性意味的普通聊天/吵架→0>
}}
...
- **connection 亲密例外**：亲密/调情/撒娇正在进行时，对方回得短是投入与回应，不是冷落——连接需求被满足，connection 应为负；只有对方敷衍、答非所问、明显不想聊、或长时间不回，才算被冷落→正。判断看对方有没有接住角色，不看字数。**此例外只影响 connection 一项：arousal/valence/desire 仍按各自准则判断，不因例外额外上调。**
- 5 个字段都必须出现（即使为 0）。
