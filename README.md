...
脚本第 ④ 步会逐项告诉你**去哪拿、填哪**，并让你当场粘贴写进文件（bot token / 你的 user_id / DeepSeek key；漏填的随时 `bash scripts/setup_keys.sh` 补），然后跑 `bash restart-bots.sh` 就上线了。管理台在 `http://127.0.0.1:8765/hub`，登录口令用 `cat ~/.claude-tgbot/hub.env` 查看。脚本可重复运行（复查用），不会覆盖你填过的配置。
